...
export function apiXhr (opts) {
  const { onAction, actionCreator, ajax, url, mockObservableAjax, ajaxObservable, epicObservable } = opts
  if (!onAction) throw new Error('missing `onAction`. provide an action like GET_STATUS, or similar')
  if (!actionCreator) throw new Error('missing actionCreator')
  if (!url && !ajax) throw new Error('`url` or `ajax` must be provided')
  if (ajax && !isObject(ajax) && !isFunction(ajax)) throw new Error('invalid ajax parameter')
  return actions$ => {
    var epic = actions$
    .ofType(onAction)
    .switchMap(action => {
      ...
      if (!ajaxConfig.url) throw new Error('epic factory missing ajax or url property')
      var ajaxObs = mockObservableAjax || Observable
      .ajax(isFunction(ajaxConfig) ? ajaxConfig(action) : ajaxConfig)
    ...
    if (!(epic instanceof Observable)) throw new Error('epic factory `observable` must return an Observable')
    return epic
  }
}
